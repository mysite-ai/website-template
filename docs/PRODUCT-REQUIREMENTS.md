...
| I want Meta Pixel tracking so I can retarget people who landed here from a Meta ad. Optional per location — some don't run ads. | Column `meta_pixel_ids` (array) on `template_locations`. Two datasets: `RestaurantsWebPixel` (`849479710958676`) for gastro, `RetailWebPixel` (`4347830552199286`) for non-gastro retail — the operator is asked which one during onboarding, see [docs/02-adding-a-client.md](./02-adding-a-client.md#which-meta-pixel). Fires `PageView`, `QRGenerated`, `Lead` events. Code: [src/lib/attribution/metaPixel.ts](../src/lib/attribution/metaPixel.ts), [src/components/attribution/MetaPixel.tsx](../src/components/attribution/MetaPixel.tsx). |
...
- **Meta Pixel** — optional per location (via `meta_pixel_ids` array). Two datasets: `RestaurantsWebPixel` for gastro, `RetailWebPixel` for non-gastro retail; chosen per tenant at onboarding so the two audience pools stay separate. Fires `PageView`, `QRGenerated`, `Lead`. See [src/lib/attribution/metaPixel.ts](../src/lib/attribution/metaPixel.ts).
